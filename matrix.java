...
public class matrix {
	...
	public static void Max(){
		...
		System.out.println("Enter No. of Column: ");
		 ...
		 //largest element in Matrix rows are insert in Array row
                int[] row =  largestInRow(N, arr, row_result); 
              //Create a Temporary array to append Sorted Array row with Matrix.
          int[][] temparr = new int[N+1][M+1];
          for(int i=0;i<N+1;i++){
        	  for(int j=0;j<M+1;j++){
          ...
          int[] col = largestInCol(M+1,temparr,col_result);
    ...
    public static int[] largestInRow(int no_of_rows, int[][] arr, int[] row_result) { 
        int i = 0; 
 ...
 // Function to get maximum element in column
    public static int[] largestInCol(int cols, int[][] temparr, int[] col_result) { 
        int i = 0; 
